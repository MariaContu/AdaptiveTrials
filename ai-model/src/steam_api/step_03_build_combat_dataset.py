...
def print_combat_subcategory_analysis(
    merged_dataframe: pd.DataFrame,
    profiles_dataframe: pd.DataFrame,
) -> None:
    """Analisa a distribuição de jogos e playtime entre subcategorias."""

    granular_column = "combat_subcategory"

    print("\n" + "=" * 60)
    print("Análise das subcategorias de combate")
    print("=" * 60)
    ...
    for subcategory in SUBCATEGORIES:
        ...
        print(
            subcategory_games.to_string(
                index=False
            )
        )


def build_combat_dataset() -> pd.DataFrame:
    """Constrói o dataset granular da categoria combat."""
    ...
    print(
        f"Registros de jogos de combate: "
        f"{len(merged_dataframe)}"
    )

    print(
        f"Perfis com jogos de combate: "
        f"{merged_dataframe['steamid'].nunique()}"
    )

    merged_dataframe["hours"] = (
        merged_dataframe[
            "playtime_forever"
        ]
        / 60
    ...
    # A análise precisa ocorrer somente depois
    # da criação das colunas hours_*.
    print_combat_subcategory_analysis(
