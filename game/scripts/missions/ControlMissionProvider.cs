...
/// <summary>
/// Prepara a sequência fixa e equilibrada utilizada no modo controle.
/// </summary>
public sealed class ControlMissionProvider
{
		...
		IReadOnlyList<MissionDto> catalog)
	{
		...
		List<MissionDto> combat =
			GetCategoryMissions(catalog, MissionType.Combat);

		List<MissionDto> exploration =
			GetCategoryMissions(catalog, MissionType.Exploration);

		List<MissionDto> puzzle =
			GetCategoryMissions(catalog, MissionType.Puzzle);
		...
		sequence.AddRange(
			SelectRandomMissions(combat, MissionsPerCategory));

		sequence.AddRange(
			SelectRandomMissions(exploration, MissionsPerCategory));

		sequence.AddRange(
			SelectRandomMissions(puzzle, MissionsPerCategory));
		...
		MissionType type)
	{
		if (missions.Count < MissionsPerCategory)
		{
			throw new InvalidOperationException(
				...
				$"{MissionsPerCategory} missões da categoria {type}.");
		}
	}

	private static IReadOnlyList<MissionDto> SelectRandomMissions(
		IReadOnlyList<MissionDto> missions,
		int amount)
	{
		List<MissionDto> available = missions.ToList();
		List<MissionDto> selected = new();

		for (int index = 0; index < amount; index++)
		{
			int selectedIndex =
				GD.RandRange(0, available.Count - 1);
	...
	private static void Shuffle(List<MissionDto> missions)
	{
		...
		IReadOnlyList<MissionDto> sequence)
	{
		...
		int combatCount =
			sequence.Count(mission =>
				mission.Type == MissionType.Combat);

		int explorationCount =
			sequence.Count(mission =>
				mission.Type == MissionType.Exploration);

		int puzzleCount =
			sequence.Count(mission =>
				mission.Type == MissionType.Puzzle);
		...
		IReadOnlyList<MissionDto> sequence)
	{
		...
		{
			MissionDto mission = sequence[index];
